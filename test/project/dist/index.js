 ...
 */
const lit_html_adapter_1 = require("./lit-html-adapter");
/**
...
const minuteTicks = (() => {
    const lines = [];
    for (let i = 0; i < 60; i++)
        lines.push(lit_html_adapter_1.svg `<line class="minor" y1="42" y2="45" transform=${`rotate(${360 * i / 60})`}/>`);
    return lines;
})();
const hourTicks = (() => {
    const lines = [];
    for (let i = 0; i < 12; i++)
        lines.push(lit_html_adapter_1.svg `<line class="major" y1="32" y2="45" transform=${`rotate(${360 * i / 12})`}/>`);
    return lines;
})();
